...
def five_tweet(client)
  handles = five_journalists
  handles.each do |i|
  client.update("#bonjour_monde #{i} Vive la liberté d'expression!")
  end
end

def five_journalists
  ...
  j_length = journalists.length.to_i
  five_journa = []
  i = 0
  while i != 5
    nb_rand = Random.new
    nb_rand = nb_rand.rand(0..j_length).to_i
    if five_journa.count(journalists[nb_rand]) == 0
      five_journa[i] = journalists[nb_rand]
      i = i + 1
    end
  end
  return five_journa
